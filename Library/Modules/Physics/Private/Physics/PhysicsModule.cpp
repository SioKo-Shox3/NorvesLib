...
namespace NorvesLib::Modules::Physics
{
    ...
    void PhysicsModule::RegisterReflectedTypes()
    {
        Core::ClassRegistry& classRegistry = Core::ClassRegistry::Get();
        const Core::IClass* colliderClass = ColliderComponent::StaticClass();
    ...
    void PhysicsModule::PreFixedTick(float fixedDeltaTime)
    {
        ...
        for (BodySlot& body : m_BodySlots)
        {
            ...
            {
                DiscardBodyStepState(body);
            }
            else if (body.bActive)
            {
    ...
    bool PhysicsModule::IsBodyLifecycleActive(const BodySlot& body) const
    {
        return body.bOccupied
            && body.Component != nullptr
            && body.Owner != nullptr
            && body.Component->IsActive()
            && body.Owner->IsActive()
            && !body.Component->HasFlag(Core::OF_PendingDestroy)
            && !body.Owner->IsPendingDestroy();
    }

    void PhysicsModule::DiscardBodyStepState(BodySlot& body)
    {
        body.PendingImpulse = Math::Vector3();
    ...
    {
        if (!body.bActive || !IsBodyLifecycleActive(body)
