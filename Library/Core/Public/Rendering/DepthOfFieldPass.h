...
namespace NorvesLib::Core::Rendering
{
     ...
     *
     * 各画素のCoCの円板で散らした寄与を集め、中心へ届く標本のうち最も手前のCoCに近いものを前景、
     * 残りを背景に分ける。前景は散らした円板の被覆の和を不透明度として背景の上に重ね（前ボケの縁の
     * 半透明）、背景は正規化した平均で手前の面の後ろの穴を埋める。中心より奥の背景の広がりは中心のCoCまでに
     * 抑える（ピントの合った手前の物体へ奥のぼけが乗らない）。PTと同じ倍率の拡大はgatherの中で部分画素の
     * 位置へ写して掛け、結果をSceneColorへ書き戻す。カメラのピント距離が0（ピンホール）か、検証表示中は
     * 何もしない。
     */
