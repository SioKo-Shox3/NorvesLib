...
namespace NorvesLib::Core::Component
{
     ...
     *
     * ピボット（注視対象の Entity）を中心とした球面座標でアームを駆動し、
     * オーナーの Entity のトランスフォーム（位置・回転）を毎フレーム更新します。
     ...
     *
     * 寿命設計:
     * - ピボットは ObjectId（PivotObjectId）で保持し、毎回 World::FindEntityByObjectId で
     *   解決します。Entity ハンドル（EntityHandle/ComponentDataRegistry）は使いません
     *   （Release 構成では ComponentDataRegistry が無効化されハンドル解決ができないため）。
     ...
     * - **ピボットは root Entity 限定です**（親を持つ Entity は非対応）。Pan は
     *   ピボットのワールド座標を読み取りローカル座標へ書き込む read-modify-write のため、
     *   root Entity（ローカル座標＝ワールド座標）でなければ座標変換が未実装で破綻します。
     *   `SetPivot` は親を持つ Entity を渡すと警告ログを出しますが、処理は継続します。
     *
     * Pan の扱い（Maya 忠実）:
     * - Pan は TargetOffset ではなく **ピボット Entity の Position** を動かします
    ...
    public:
         ...
         *   root Entity 限定（親を持つ Entity を渡すと警告ログを出しますが処理は継続します）。
         *   owner 自身（このコンポーネントの GetOwner()）を渡した場合は不動点が
         *   存在せず毎フレームドリフトするため、警告ログを出したうえで設定を拒否します
         *   （PivotObjectId は変更されません）。
         */
        void SetPivot(const Entity *pivot);
        ...
        /**
         * @brief 現在ピボットが解決可能か（owner→World→FindEntityByObjectId が非null）
         */
        bool HasValidPivot() const;

        /**
         * @brief 現在のピボット Entity を解決して返す（解決不可なら nullptr）
         */
        ...
        /**
         * @brief 現在のアーム状態から owner Entity の Transform を即座に再計算して書き込む
         *
         * 現在の (ArmLength / Yaw / Pitch) と解決済みピボットから、owner Entity の
