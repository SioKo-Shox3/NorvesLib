...
namespace NorvesLib::Core::Component
{
    ...
    void SpringArmComponent::SetPivot(const Entity *pivot)
    {
        if (pivot != nullptr && pivot == GetOwner())
        {
            // owner 自身をピボットにすると、Tick 毎に
            // target = pivot->GetPosition()（= owner の現在位置）
            // owner->SetPosition(target + ComputeArmOffset())
            // という自己参照になり、入力なしでも毎フレーム同じオフセット分だけ
            // カメラがドリフトし続ける（不動点が存在しない）。数学的に無効なため、
            // 警告を出したうえで設定自体を拒否する（PivotObjectId は変更しない）。
            NORVES_LOG_WARNING("SpringArmComponent",
                               "Pivot (ObjectId=%llu) is the same Entity as the owner; "
                               "self-pivot is rejected because it has no fixed point and would drift "
                               "every frame",
                               static_cast<unsigned long long>(pivot->GetObjectId()));
            return;
        }

        if (pivot != nullptr && pivot->GetParentEntity() != nullptr)
        {
            // Pan はピボットのワールド座標を読み取りローカル座標へ書き込む
            // read-modify-write のため、root Entity（ローカル座標＝ワールド座標）
            // でなければ座標変換が未実装で破綻する。警告のみで処理は継続する。
            NORVES_LOG_WARNING("SpringArmComponent",
                               "Pivot (ObjectId=%llu) has a parent Entity; SpringArmComponent only "
                               "supports root Entity pivots (Pan world/local conversion is not implemented "
                               "for child Entities)",
                               static_cast<unsigned long long>(pivot->GetObjectId()));
        }
    ...
    Entity *SpringArmComponent::ResolvePivot() const
    {
        ...
        return world->FindEntityByObjectId(PivotObjectId);
    }
    ...
    {
        // 注意: intent.bHasInput はここではチェックしない。各 Delta が 0 の
        // intent を渡しても無害（Yaw/Pitch/ArmLength は不変、Pan は非ゼロ判定でスキップ）
        // なため、ゲートするかどうかは呼び出し側（GameMode 等）の責務とする設計。
        ...
        // ピボット Entity の Position を動かす（焦点移動＝Maya 忠実）。
