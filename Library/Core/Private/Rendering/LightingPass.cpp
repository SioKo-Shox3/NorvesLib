...
namespace NorvesLib::Core::Rendering
{
                                        ...
                                        uint32_t height)
    {
        ...
        for (size_t pixel = 0u; pixel < pixelCount; ++pixel)
        {
            for (uint32_t channel = 0u; channel < 3u; ++channel)
            {
                ...
                if (!TryScaleEnvironmentSourceValue(value, 1.0, validatedValue))
                {
                    return false;
                }
    ...
    static bool ValidateProductionInitializationInvariants(float scale)
    {
        ...
        for (const SourcePredicateCase& sourceCase : sourcePredicateCases)
        {
            ...
            const bool bActualValid =
                TryScaleEnvironmentSourceValue(sourceCase.fileValue,
        ...
        Container::VariableArray<RHI::BufferPtr>& outGeometryBuffers)
    {
        ...
        for (const RayTracingSceneInstanceSnapshot& snapshot : scene.Instances)
        {
            ...
            // 材質のBaseColorは使わない）で作る。RTGIはtextureを引かないので白とみなす。
            for (uint32_t channel = 0u; channel < 4u; ++channel)
            {
                if (!IsFiniteNonNegativeRTGI(snapshot.Material.ObjectColor[channel]))
                {
                    return false;
                }
    ...
    {
        // ラスタとパストレーサーが同じ値の環境を使うよう、読み込みと換算は共有関数に任せる。
        Container::VariableArray<float> sourceData;
        uint32_t sourceWidth = 0u;
        uint32_t sourceHeight = 0u;
        if (!LoadEnvironmentRadianceSource(path, m_Settings.EnvironmentLuminanceScaleNits,
