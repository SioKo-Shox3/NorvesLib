...
- **実装担当とレビュー担当は必ず別エージェント**にする。さらに **レビューは Codex と Claude Code の両方**で行い（二重レビュー）、必要な場面では **Codex にも実装を依頼**する（詳細は後述「Codex との連携」）。
- 各工程は独立してレビュー・検証・コミットできる最小単位にする。クリーンに分割できない場合は理由を述べ、最小の実用的チェックポイントを定義する。先行する依存フェーズが実装レビューと検証ゲートを通過するまで、後続フェーズの実装を始めない。

### Codex との連携（標準フロー・必須）

このリポジトリでは Claude Code（オーケストレーター/実装/レビュー）と **Codex を常時連携**させる。以下を既定フローとする。

- **レビューは必ず Codex と Claude Code の両方で行う（二重レビュー）。** 計画レビュー・実装レビューのどちらも、独立した 2 系統（Codex 1 名 + Claude サブエージェント 1 名）で並行実施し、両者の指摘を突き合わせてからオーケストレーターが統合判定する。片方だけのレビューで次工程へ進めない。
- レビューは独立性を保つため、各レビュアーに**他方の所見を事前に渡さない**（同じ計画/diff と同じ観点リストを渡し、独立に出させてから統合する）。
- **実装も必要に応じて Codex に依頼する。** 難所（所有権/寿命、RHI/Vulkan、スレッド/同期、Public API、アセットロード等）や第二の実装視点が欲しい場面で Codex に実装フェーズを割り当ててよい。「実装担当とレビュー担当は別主体」の原則は Codex を使う場合も維持する（Codex 実装なら少なくとも Claude を、Claude 実装なら少なくとも Codex をレビュー系統に含める）。

**Codex の起動方法（companion ランタイム）**

- セットアップ確認: `node "<codex-companion.mjs>" setup --json`（現状の実体は `~/.claude/plugins/cache/openai-codex/codex/<ver>/scripts/codex-companion.mjs`）。
- レビュー/タスク起動: `... task --background --fresh --effort high "<prompt>"`。diff レビューは `review` / `adversarial-review` も使える。
- **長時間ジョブは `--background` で起動し、`status [job-id] --all --json` でポーリング、`result [job-id] --json` で結果取得**する（`--wait` は Bash タイムアウトに当たりやすい）。
- **Codex には「ローカルリポジトリを直接読む / GitHub 検索ツール（github/search_code 等）を使わない」と明示する。** 指示しないと存在しない GitHub を検索し続けて停滞することがある（実観測）。
- **サンドボックスがローカルのファイル/シェルを一切読めない場合がある**（`windows sandbox: spawn setup refresh` 等。`.claude/worktrees/` 配下で観測。`--write` でも解消しないことがある）。その場合は Codex にファイルを読ませようとせず、**レビュー/タスク対象（計画文書・関連実コードの抜粋を file:行 付き）を 1 つの md に集約し `--prompt-file <md>` で丸ごと渡す**（companion 側 Node がファイルを読むのでサンドボックス制約を回避できる）。`--prompt-file` は長文をコマンド長制限なしで渡せる利点もある。
- ジョブ停止は Windows のパス変換で `cancel` が失敗することがある。その場合は PowerShell から `taskkill /PID <pid> /T /F` で停止する。
- `codex:codex-rescue` サブエージェント（Agent ツール）も利用可だが、上記 companion 直叩きの方が状態を制御しやすい。
...
2. **計画 (Planning)** — フェーズごとに具体的な実装計画を書く。計画には次を含める:
   ...
   - コミット境界（コミット可能になる条件）。
3. **計画レビュー (Plan Review)** — **Codex と Claude Code の二重レビューで**、API 境界、所有権モデル、依存方向、リソース寿命、スレッド安全性、CMake/テスト登録、後方互換、コミット単位の妥当性を確認。エンジンクリティカルな変更（Public API・RHI/Vulkan・RenderThread 挙動・メモリ管理・ジョブスケジューリング・アセットロード・シェーダーパイプライン・Object/Resource 寿命）は、変更が小さく見えても必須。
...
5. **実装レビュー (Implementation Review)** — **Codex と Claude Code の両方で**実 diff を承認済み計画と突き合わせ、公開 API 形状、所有権/破棄経路、依存方向、共有ヘッダ影響、生成/ビルドファイルの除外、CMake 登録、テストカバレッジ、サブエージェント出力を確認。指摘は次に進む前に修正する。計画した検証コマンドを実行できなかった場合は理由と残存リスクを記録する。
