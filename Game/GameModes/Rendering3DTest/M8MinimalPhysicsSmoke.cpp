...
namespace Game::GameModes
{
    ...
    namespace
    {
        ...
        constexpr float kMaximumSnapshotTranslation = 0.01f;
    ...
    bool M8MinimalPhysicsSmoke::Enter(NorvesLib::Core::GameMode::GameModeContext& ctx)
    {
        ...
        m_LastObservedRenderedFrame = 0;
        m_LastPublishedSnapshotSequence = 0;
        m_StableObservationCount = 0;
        m_bComplete = false;
        m_bObservedGravityProgress = false;
        m_bHasSnapshotBaseline = false;

        auto& world = ctx.WorldRef;
        m_pFloorEntity = ctx.ScopeRef.SpawnObject<Entity>();
        m_pBoxEntity = ctx.ScopeRef.SpawnObject<Entity>();
        m_pSphereEntity = ctx.ScopeRef.SpawnObject<Entity>();
        m_pCapsuleEntity = ctx.ScopeRef.SpawnObject<Entity>();
    ...
    void M8MinimalPhysicsSmoke::Update(NorvesLib::Core::GameMode::GameModeContext& ctx)
    {
        ...
        if (ctx.EngineRef.GetSceneQuery().GetPublishedSnapshotSequence(publishedSnapshotSequence)
                != NorvesLib::Core::Scene::EPhysicsSceneQueryResult::Success
            || publishedSnapshotSequence == 0)
        {
            m_StableObservationCount = 0;
            return;
        }
        if (publishedSnapshotSequence == m_LastPublishedSnapshotSequence)
        {
            return;
        }
        ...
        if (!m_bHasSnapshotBaseline)
        {
            m_PreviousBoxPosition = boxPosition;
            m_PreviousSpherePosition = spherePosition;
            m_PreviousCapsulePosition = capsulePosition;
            m_LastPublishedSnapshotSequence = publishedSnapshotSequence;
            m_bHasSnapshotBaseline = true;
            ObserveGravityProgress();
            return;
        }
    ...
    bool M8MinimalPhysicsSmoke::ObserveStack() const
    {
        ...
        return IsFiniteVector(boxPosition) && IsFiniteVector(spherePosition) && IsFiniteVector(capsulePosition)
            ...
            && (boxPosition - m_PreviousBoxPosition).LengthSquared()
                <= kMaximumSnapshotTranslation * kMaximumSnapshotTranslation
            && (spherePosition - m_PreviousSpherePosition).LengthSquared()
                <= kMaximumSnapshotTranslation * kMaximumSnapshotTranslation
            && (capsulePosition - m_PreviousCapsulePosition).LengthSquared()
                <= kMaximumSnapshotTranslation * kMaximumSnapshotTranslation
