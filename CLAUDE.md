...
> **この文書は `AGENTS.md` と `CLAUDE.md` で完全に同一内容で運用する。** Claude Code は `CLAUDE.md`、Codex は `AGENTS.md` を読むが、中身は常に揃える。一方を編集したら必ずもう一方へ同じ変更を反映すること。外部ファイル（旧 `.github/copilot-instructions.md` 等）は参照しない。この文書が唯一の一次情報。
...
- **実装は既定で Codex に委譲する**（Codex は使用量に余裕があるため、Claude の実装枠を温存できる）。Claude 自身の実装は、Codex で手戻りが続く難所や、Codex に文脈を渡すより速い小修正、共有ファイルの順次編集に限定する。
- **レビューは Claude＋Codex のダブルチェック**にする。計画レビュー・実装レビューとも一次を最上位 Claude（実装者と別エージェント）、二次を独立した Codex が行う。詳細は「Codex への委譲とレビューのダブルチェック」を参照。
- 各工程は独立してレビュー・検証・コミットできる最小単位にする。クリーンに分割できない場合は理由を述べ、最小の実用的チェックポイントを定義する。先行する依存フェーズが実装レビューと検証ゲートを通過するまで、後続フェーズの実装を始めない。

### Codex への委譲とレビューのダブルチェック

実装は基本的に Codex に投げ、レビューは Claude と Codex の二重化で品質を担保する。Codex は使用量に余裕があるため、Claude の実装枠を温存しつつスループットを上げられる。これは下の「工程の流れ」「モデル選定」に優先して適用する既定方針。

- **実装の既定担当は Codex**: 定型〜中規模の実装フェーズは Codex CLI（codex プラグイン、`codex:rescue` / Codex ランタイム）に委譲する。Claude 自身が実装するのは、(a) Codex で手戻りが続く難所、(b) Codex に文脈を渡すより自分で直した方が速い小修正、(c) 公開ヘッダ・中央 CMake・コア寿命管理など共有ファイルでオーケストレーターが順次編集すべき箇所、に限る。
- **委譲時に必ず渡す情報**: 対象フェーズと目的、許可/禁止の書き込みパス、本書「コーディング規約」の独自型ルール（コンテナ/スマポ/ログ/計測）、行末・BOM ルール（既存は CRLF 維持、新規ソースは UTF-8+BOM+CRLF）、期待する出力形式（変更ファイル・実行した検証・未解決リスク・前提）。Codex の出力も「提案物」として扱い、オーケストレーターが diff を統合観点で検査してから受け入れる。
- **レビューはダブルチェック**: 計画レビュー・実装レビューとも、一次レビューを最上位 Claude（実装者とは別エージェント）が行い、加えて二次レビューを独立した Codex が行う。両者の指摘を突き合わせ、片方だけが挙げた指摘も解消してから次工程へ進む。
- **自己レビュー禁止（独立性の担保）**: 実装した本人（同一エージェント・同一セッション）に自分の変更をレビューさせない。Codex が実装した変更は、Claude 一次レビュー＋クリーンな文脈の別 Codex レビューで二重化し、自己レビューにならないようにする。
- **エンジンクリティカルな変更**（Public API・RHI/Vulkan・RenderThread・メモリ・ジョブ・アセットロード・Object/Resource 寿命）は、Codex 委譲時もレビューのダブルチェック（Claude 一次＋Codex 二次）を省略しない。
...
2. **計画 (Planning)** — フェーズごとに具体的な実装計画を書く。計画には次を含める:
   ...
   - コミット境界（コミット可能になる条件）。
3. **計画レビュー (Plan Review)** — API 境界、所有権モデル、依存方向、リソース寿命、スレッド安全性、CMake/テスト登録、後方互換、コミット単位の妥当性を確認。エンジンクリティカルな変更（Public API・RHI/Vulkan・RenderThread 挙動・メモリ管理・ジョブスケジューリング・アセットロード・シェーダーパイプライン・Object/Resource 寿命）は、変更が小さく見えても必須。一次レビュー（最上位 Claude）の後、Codex による独立した二次レビュー（ダブルチェック）も通す。
4. **実装 (Implementation)** — 承認された計画に沿って編集。**実装は既定で Codex に委譲する**（codex プラグイン経由）。Codex に渡すタスクには対象フェーズ・目的・許可/禁止の書き込みパス・独自型ルール・行末/BOM ルール・期待する出力を明示し、出力は提案物として統合前に diff を検査する。サブエージェントには支援するフェーズと目的、許可/禁止の書き込みパス、期待する出力形式（変更ファイル・実行した検証・未解決リスク・前提）、タスク種別（read-only/実装/テスト作成/レビュー）を明示する。書き込みスコープはサブエージェント間で重複させない。公開ヘッダ・中央 CMake・ビルド設定・シリアライズスキーマ・コア寿命管理など共有ファイルは単一オーナーかオーケストレーターが順次編集する。
5. **実装レビュー (Implementation Review)** — 実 diff を承認済み計画と突き合わせ、公開 API 形状、所有権/破棄経路、依存方向、共有ヘッダ影響、生成/ビルドファイルの除外、CMake 登録、テストカバレッジ、サブエージェント出力を確認。指摘は次に進む前に修正する。計画した検証コマンドを実行できなかった場合は理由と残存リスクを記録する。この一次レビュー（最上位 Claude）に加え、Codex による独立した二次レビュー（ダブルチェック）を通し、両者の指摘を突き合わせてから次へ進む。実装者と同一エージェントに自己レビューさせない（Codex 実装分は Claude 一次＋クリーン文脈の別 Codex で二重化する）。
...
| 実装 | **既定で Codex に委譲**（使用量に余裕があるため Claude 枠を温存）。Claude で実装する場合は下位、昇格条件に該当するなら上位〜最上位へ。 |
...
> **実装の既定の実行者は Codex**（codex プラグイン）。Claude の実装枠は難所・小修正・共有ファイルの順次編集に温存し、レビューは Claude 一次＋Codex 二次のダブルチェックにする（→「Codex への委譲とレビューのダブルチェック」）。
