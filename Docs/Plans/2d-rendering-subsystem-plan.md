...
> **【重要・時制の規約】** 本計画は **「現状（実コード）」** と **「実装後の目標状態」** を明確に分ける。F1 landed 後の現状では `CanvasView`/`CompositePass`/`Canvas.Color`/`Composite.Color`/`RenderFrameExecutor` 2 段化は**既に存在する合成アンカー**であり、F2 はその上にカメラ表・CameraId 参照・Canvas 正射影 Viewport を足す。`BoardComponent`/`BoardProxy`/`BoardSpace`/カメラ表/`Viewport` の `CameraId` 参照/`ProceduralMeshGenerator::GenerateQuad2D`/`UVRect` は引き続き後続フェーズの実装対象である。
>
> - **§2（現状整理）** は **F0/F1 landed 後に確認される実コードのベースライン**を記述する。ここに現れる型・関数・file:line は**すべて既存**（`View`/`SceneView`/`CanvasView`/`Viewport`/`CameraProxy`/`MeshProxy`/`MeshComponent`/`World::SyncEntityRecursive`/`RenderingCoordinator::GenerateDrawCommands`/`PresentationPass`/`CompositePass`/`RenderFrameExecutor`/`RenderGraphResourceNames`）。**§2 に未実装機能の断定を混ぜない。** 「現状こうなっている（だからここに足す）」という読み取り専用記述に統一する。
> - **§3 以降（目標アーキテクチャ・フェーズ計画）** は **「実装後にこうなる（will be）」** という未来時制で読む。ただし F1 landed 済みの合成アンカーは既存として扱い、F2 以降で新設する型・関数・named resource だけを未来形で扱う。各 §3 項目は、どの §2 既存項目から拡張するかを明示する。
...
NorvesLib は F1 landed 後の現状で **3D の Deferred + Forward 透過パイプラインに加え、空 Canvas 合成アンカー**（`CanvasView`/`CompositePass`/`Canvas.Color`/`Composite.Color`/`RenderFrameExecutor` 2 段化）を持つ。ただし、2D オーサリング機能（スプライト/ビルボード/インポスター/テクスチャアニメーション/スプライトシート分割）はまだ無く、`SceneProxy.h` には `MeshProxy`/`MegaGeometryProxy`/`LightProxy`/`CameraProxy` のみ、`DrawCommandType` は 2D 用ペイロードを持たない。
...
調査結果（実コードで検証済み）に基づく**現状（既存コード）**。これらが唯一の現状根拠であり、調査に無い断定はしない。**本節に列挙する型・関数・file:line はすべて現在のコードベースに存在するもの**で、F1 landed 済みの `CanvasView`/`CompositePass`/`Canvas.Color`/`Composite.Color`/`RenderFrameExecutor` 2 段化を含む。一方、`BoardComponent`/`BoardProxy`/`BoardSpace`/カメラ表 等の**未実装機能は含まない**（それらは §3 の実装後目標）。各小節末の「→ ここに足す」注記は、実装で拡張する**既存の足場**を指す。

> **未実装の確認（F1 landed 後）:** 次は現在のコードに存在しない（本計画の新設対象）— `BoardComponent`（`Component` 派生）, `BoardProxy`（`SceneProxy.h`）, `BoardSpace` enum（`RenderTypes.h`）, `RenderingCoordinator` のカメラ表（`RegisterCamera`/`FindCamera`）, `Viewport` の `CameraId` 参照, `ProceduralMeshGenerator::GenerateQuad2D`, `UVRect`（型自体が未定義）。`CanvasView`/`CompositePass`/`Canvas.Color`/`Composite.Color`/`RenderFrameExecutor` 2 段化は F1 で導入済み。`BlendMode` は既存だが `MaterialTypes.h:198`（`RenderTypes.h` ではない）。
...
> **【未実装の明示】** 本図は **実装後の確定アーキテクチャ**を示す。F1 landed 後の現状では `CanvasView`/`CompositePass`/`Canvas.Color`/`Composite.Color`/2 段化 `RenderFrameExecutor` は既存の合成アンカーである。`BoardComponent`/`BuildBoardProxy`/`BoardProxy`/カメラ表は F2-F3 以降の実装対象であり、図中で既存（`MeshProxy`/`LightProxy`/`SyncEntityRecursive`/`GenerateDrawCommands`/`PresentationPass`）の隣に並べているのは、**実装後の最終形**を一望するためである。現状（§2）では `BoardComponent` 分岐は存在しない。
>
...
- 新規 `Library/Core/Public/Rendering/CanvasView.h` / `Private/Rendering/CanvasView.cpp`: `CanvasView : public View`、`Initialize` で `ViewType::UI`、`Render(ViewRenderContext&)` を**完全オーバーライド**（基底を呼ばず `Reset` → 空（透明クリア）の `m_OutputTexture` を確定し `Canvas.Color` publish → `PresentationGraphPass` 追加しない、§3.6.1）。透明 Canvas は RenderGraph pass を実行し、null-pipeline fullscreen/render-pass command 等で `BeginRenderPass`/`EndRenderPass` の clear/store を物理発行する（宣言のみ不可）。F1 landed 後の CanvasView は full-rect Viewport を 1 つ持つ不変条件で扱い、F2 でその Viewport に CameraId と正射影 value camera を coordinator から割り当てる。
...
`CameraProxy.CameraId`（`SceneProxy.h:259`）を有効化し、`RenderingCoordinator` にカメラ表（`RegisterCamera`/`UpdateCamera`/`FindCamera`）を導入する。Viewport は CameraId 参照で共有カメラを引く（値所有は維持、スナップショット値コピーも維持）。F1 landed 済みの CanvasView は full-rect Viewport を 1 つ持つ Canvas invariant とし、F2 で coordinator がその Viewport に正射影カメラ（`CullingMask=RenderLayer::UI`、`OrthoWidth=m_RenderWidth`、`OrthoHeight=m_RenderHeight`）を登録・割当・リサイズ追従させる。
