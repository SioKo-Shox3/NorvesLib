...
対象ブランチ: `develop`
重点領域: `Library/Core/Public/Component/BoardComponent.h` / `Private/Component/BoardComponent.cpp`（新規）, `Library/Core/Public/Rendering/SceneProxy.h`（`BoardProxy` 追加）, `Library/Core/Public/Object/World.h` / `Private/Object/World.cpp`（`SyncEntityRecursive` 分岐 + マスクルーティング）, `Library/Core/Public/Rendering/SceneView.h` / `Private/Rendering/SceneView.cpp`, 新規 `Library/Core/Public/Rendering/CanvasView.h` / `Private/Rendering/CanvasView.cpp`, `Library/Core/Public/Rendering/RenderingCoordinator.h` / `Private/Rendering/RenderingCoordinator.cpp`（カメラ表 + ルーティング + GenerateDrawCommands）, `Library/Core/Public/Rendering/RenderFrameExecutor.h` / `Private/Rendering/RenderFrameExecutor.cpp`, `Library/Core/Public/Rendering/PresentationPass.h` / `Private/Rendering/PresentationPass.cpp`, 新規 `Library/Core/Public/Rendering/CompositePass.h` / `Private/Rendering/CompositePass.cpp`, `Library/Core/Public/Rendering/RenderGraph/RenderGraphResourceNames.h`, `Library/Core/Public/Rendering/Viewport.h` / `Private/Rendering/Viewport.cpp`, `Library/Core/Public/Rendering/ViewRenderPlan.h`, `Library/Core/Public/Rendering/FramePacket.h`, `Library/Core/Public/Rendering/DrawCommand.h`, `Library/Core/Public/Rendering/ProceduralMeshGenerator.h`, `Library/Core/Public/Rendering/VertexLayout.h`, `Assets/Shaders/`（新規 2D シェーダー）, `Library/Core/CMakeLists.txt`, `Docs/Architecture/RenderingFlow.md`
...
> **【重要・時制の規約】** 本計画は **「現状（実コード）」** と **「実装後の目標状態」** を明確に分ける。`BoardComponent`/`BoardProxy`/`BoardSpace`/`CanvasView`/`CompositePass`/`Canvas.Color`/`Composite.Color`/カメラ表/`RenderFrameExecutor` 2 段化 は**現在のコードベースに一切存在しない**（`grep` 確認済み: いずれも 0 ヒット）。これらは本計画の**実装対象（F0 で正確な追加点を確定 → F1 以降で新設）**である。
>
> - **§2（現状整理）** は **F0 完了時点で確認される実コードのベースライン**を記述する。ここに現れる型・関数・file:line は**すべて既存**（`View`/`SceneView`/`Viewport`/`CameraProxy`/`MeshProxy`/`MeshComponent`/`World::SyncEntityRecursive`/`RenderingCoordinator::GenerateDrawCommands`/`PresentationPass`/`RenderFrameExecutor`/`RenderGraphResourceNames`）。**§2 に未実装機能の断定を混ぜない。** 「現状こうなっている（だからここに足す）」という読み取り専用記述に統一する。
> - **§3 以降（目標アーキテクチャ・フェーズ計画）** は **「実装後にこうなる（will be）」** という未来時制で読む。新設型・新設関数・新設 named resource は本計画で初めて導入する。各 §3 項目は、どの §2 既存項目から拡張するかを明示する。
...
NorvesLib は現状 **3D の Deferred + Forward 透過パイプラインのみ**を持ち、2D 描画機能（スプライト/ビルボード/インポスター/テクスチャアニメーション/スプライトシート分割/2D レイヤー合成）が **一切存在しない**（`SceneProxy.h` には `MeshProxy`/`MegaGeometryProxy`/`LightProxy`/`CameraProxy` のみ。`RenderGraphResourceNames.h:7-20` の named resource は 3D Deferred コアのみで `Composite.*`/`Canvas.Color` は無い。`DrawCommandType` は 2D 用ペイロードを持たない）。
...
2. **World 同期で `BoardProxy` を生成。** `World::SyncEntityRecursive`（`World.cpp:921-1062`）が `MeshProxy`/`LightProxy` を作るのと同様に `BoardProxy` を作る（ObjectId/ComponentId キーイング、live-set/RemoveStale を踏襲）。2D 専用バイパスでなく既存の World→Proxy 機構に載せる。
...
- **RenderThread は live な `World`/`Entity`/`BoardComponent`/`View`/`Viewport` を読まない。** 2D データは GameThread が `SyncEntityRecursive` で BoardProxy 化 → ルーティング → `GenerateDrawCommands` で対象 View の `ViewportRenderPlan` へ instance/draw としてオーサリングし、RenderThread は `ViewRenderContext` のスナップショットスライス（`CurrentViewport` + 共有 `FramePacket.DrawCommands`/`InstanceData`）のみ読む。
...
調査結果（実コードで検証済み）に基づく**現状（既存コード）**。これらが唯一の現状根拠であり、調査に無い断定はしない。**本節に列挙する型・関数・file:line はすべて現在のコードベースに存在するもの**で、`BoardComponent`/`BoardProxy`/`CanvasView`/`CompositePass`/`Canvas.Color`/`Composite.Color`/カメラ表 等の**未実装機能は一切含まない**（それらは §3 の実装後目標）。各小節末の「→ ここに足す」注記は、実装で拡張する**既存の足場**を指す。

> **未実装の確認（F0 で再確認）:** 次は現在のコードに存在しない（本計画の新設対象）— `BoardComponent`（`Component` 派生）, `BoardProxy`（`SceneProxy.h`）, `BoardSpace` enum（`RenderTypes.h`）, `CanvasView`（`View` 派生）, `CompositePass`（`IRenderGraphPass`）, `Canvas.Color`/`Composite.Color`（named resource）, `RenderingCoordinator` のカメラ表（`RegisterCamera`/`FindCamera`）, `Viewport` の `CameraId` 参照, `RenderFrameExecutor` の 2 段化, `ProceduralMeshGenerator::GenerateQuad2D`, `UVRect`（型自体が未定義）。`BlendMode` は既存だが `MaterialTypes.h:198`（`RenderTypes.h` ではない）。
...
- `Viewport` は `CameraProxy m_Camera` を**値所有**（`Viewport.h:87` の `GetCamera()`、`SetCamera(const CameraProxy&)` は `Viewport.h:82` 宣言・値コピー）。自前の view/projection 行列をキャッシュ（`Viewport.h:92-102`）。コピー禁止（`Viewport.h:55-56`）。
- `CameraProxy` は pure data（`SceneProxy.h:257-290`）: `CameraId`（`:259`、現状未使用）、Position/Forward/Up/Right、`ProjectionType Projection`（`:268`、Perspective/Orthographic）、FieldOfView/OrthoWidth/OrthoHeight（`:269-274`）、`ViewportRect Viewport`（`:277`）、`RenderLayer CullingMask = RenderLayer::All`（`:280`）、`RenderOrder`（`:281`）。
- `m_MainCamera` は `CameraProxy`（`RenderingCoordinator.h:354`）、`GetMainCamera()`（`:224`）、`SetMainCamera()`（`:219`、実装 `RenderingCoordinator.cpp` 内で `m_MainCamera` へ値コピー + 主 Viewport へ `SetCamera` コピー）。`m_bCameraSet`（`:355`）。
...
- **マスク交差フィルタは新規ロジック（実装スコープの明示）:** 現状 `SceneView::CullProxies` は錐台/距離カリングのみで、`RenderLayer × CullingMask` 判定はどこにも存在しない。本計画の交差フィルタ（§3.5）は **F3 で `BoardProxy` ルーティングとして GameThread 側に新設**する（**RenderThread の描画パスには入れない**）。GameThread が proxy → View 割り当てを決め、RenderThread は事前にルーティング済みの不変 `ViewportRenderPlan` スナップショットのみ受け取る。「CullingMask が既にあるからすぐ使える」のではなく、**判定ロジックそのものを足す**点に注意（実装規模を過小評価しない）。

### 2.5.1 per-viewport オーサリングと DrawCommand 構築

- `RenderingCoordinator::GenerateDrawCommands`（`RenderingCoordinator.cpp:802-915`）: `m_Screen.GetViews()` を View ループ（`:804`）、`DynamicPointerCast<SceneView>`（`:818`）、Viewport ループ（`:855`）、`BuildViewportRenderPlan`（`:865`）、`PrepareDrawCommandsForViewport`（`:874`）→ `AppendRebasedDrawCommands`（`:880-887`）で `FramePacket.DrawCommands` へ append、`viewPlan.Viewports.push_back`（`:902`）、`packet->Views.push_back`（`:907`）。**現状この View ループは `DynamicPointerCast<SceneView>` が成功した View のみ処理し、それ以外の View 種別は無視される。**
  - **CanvasView 分岐の正確な挿入点（実装後）:** View ループ（`:804`）内 Viewport ループ（`:855`）で、`sceneView` cast 成功時の既存 3D 経路（`:872-900`）に対し、**`else if (auto canvasView = DynamicPointerCast<CanvasView>(view))` 分岐を並置**し、その中で `canvasView->PrepareBoardDrawCommands(viewportPlan)` を呼ぶ。**3D を先に append → 2D を後に append** することで `FramePacket.DrawCommands` の index が「3D 範囲 → 2D 範囲」の順に安定する（§2.5.1 末・§3.7）。行番号は F0/F1 完了時点の予測値で、実装時に再確認する。
...
- `PresentationPass::Declare`（`PresentationPass.cpp:22-40`）は現状 `TryReadTexture` 優先順 **(1) PresentationColor → (2) ToneMappedColor**（`:27-40`）。
- `RenderGraphResourceNames.h`（`:7-20`）に `GBuffer.*`/`Scene.Color`/`Scene.Depth`/`ToneMappedColor`/`PresentationColor` 等。**`Composite.Color`/`Canvas.Color` は無い**（本計画で追加）。
- `DrawCommandSorter` 系の `std::sort` は **非安定**（`DrawCommand.cpp:374/397/420`）。2D の painter 順安定性は CanvasView 側で明示安定ソートする。
- `ProceduralMeshGenerator`（`ProceduralMeshGenerator.h`）は `GenerateUVSphere`（`:34`）/`GeneratePlane`（`:158`）のみ。**2D quad 生成は無い**（本計画で `GenerateQuad2D` を追加）。`GeneratePlane` の winding は F0 で実コード確認し、`GenerateQuad2D` は **3D 規約と一致させる**（§3.3・§2.6 注）。
- **`GenerateQuad2D` の winding（実装後・F3）:** Vulkan のフロントフェイス規約（カメラから見て CCW＝反時計回りを表面）に合わせる。インデックスは 2 三角形で `0-1-2, 1-3-2`（頂点 0=左下,1=右下,2=左上,3=右上 を CCW で構成）。`GeneratePlane` の既存 winding を F0 で実測し、不整合があれば quad 側を Vulkan 規約に正準化する（深度なし 2D では裏表両面でも可だが、3D world-space Billboard と共用するため規約を統一する）。
...
> **【未実装の明示】** 本図は **実装後の確定アーキテクチャ**を示す。`BoardComponent`/`BuildBoardProxy`/`BoardProxy`/`CanvasView`/`CompositePass`/`Canvas.Color`/`Composite.Color`/カメラ表/2 段化 `RenderFrameExecutor` は**現在のコードに存在しない F0-F3/F1 の実装対象**であり、図中で既存（`MeshProxy`/`LightProxy`/`SyncEntityRecursive`/`GenerateDrawCommands`/`PresentationPass`）の隣に並べているのは、**実装後の最終形**を一望するためである。現状（§2）では `BoardComponent` 分岐も CanvasView 分岐も存在しない。
>
...
> - **Step1（`World::SyncEntityRecursive`、GameThread）:** `BoardComponent` を proxy 化するのみ。`BuildBoardProxy` で `LayerMask`/`Space`/`ObjectId`/`ComponentId` を埋め、**カメラ非依存**で `BoardProxy` を生成。F3 では「主 SceneView + 1 CanvasView」の 2 View 固定なので、`Space==ScreenSpace` を CanvasView の `BoardProxy` ストアへ、`Space==WorldSpace`（F9）を SceneView の `BoardProxy` ストアへ、**Space だけで**振り分ける（この時点でマスク交差は見ない）。live-set 登録・stale 回収もここ。
> - **Step2（`RenderingCoordinator::GenerateDrawCommands`、GameThread）:** View ループ（`:804`）で当該 View の Viewport の `Camera.CullingMask` を読み、各 `BoardProxy.LayerMask` と `HasFlag` 交差判定（§3.5）。**交差した Board のみ** `PrepareBoardDrawCommands` で当該 viewport の DrawCommand へ append。交差しない Board はストアに残るが描画しない（live-set からは外さない＝stale 回収対象外）。
>
      ...
      SyncEntityRecursive(entity, materials, liveSets, cdr)  （:921）
        for comp in entity.GetComponents():
          ...
          CastTo<BoardComponent>  → BuildBoardProxy  → BoardProxy （新規, カメラ非依存）
            ...
            ─ Step1: Space だけで proxy ストアへ振り分け（GameThread, カメラ非依存）─
              Space==ScreenSpace → CanvasView の m_BoardProxies へ UpdateBoardProxy
              Space==WorldSpace  → SceneView の m_BoardProxies へ UpdateBoardProxy（F9）
              （CanvasView 未登録時は ScreenSpace Board をストアに保持するだけで描かない）
              キー: ObjectId/ComponentId、live-set 登録
      RemoveStaleBoardProxies(liveBoardIds)   ← BoardProxy も live-set 差分回収    （:870-872 に追加）
  ...
  RenderingCoordinator::GenerateDrawCommands()              （RenderingCoordinator.cpp:802）
    ...
    for view in m_Screen.GetViews():                        （:804）
      DynamicPointerCast<SceneView>  → 既存 3D 経路（:872-900、不変）
        ...
        → quad draw/instance を FramePacket.DrawCommands/InstanceData へ append（3D の後）
   ...
   ※ シグネチャは Execute(const RenderFrameExecutionRequest&) const。request.Packet/Views/Context/...
    if (bUseCompositePass):   ← packet ViewId から enabled CanvasView を解決できた時だけ true（§3.6.5）
      段1: 各 View を Priority 昇順で自前の中間 RT（View::m_OutputTexture）へ描く（present しない）
        ...
        CanvasView::Render(context) → Board painter 描画を CanvasView::m_OutputTexture へ store（基底完全オーバーライド）
...
protected:
    ...
    PROPERTY(Rendering::UVRect, UVRectProp)         // スプライトシート UV（F6）
...
### 3.3 BoardProxy（MeshProxy と同型・ObjectId/ComponentId キーイング）
...
struct BoardProxy
{
    ...
    TextureHandle Texture;          // 参照カウント Resource
    ...
    UVRect UV = UVRect::FullTexture(); // [0,1] 既定
...
`enum class BoardSpace : uint8_t { ScreenSpace = 0, WorldSpace }` を `RenderTypes.h` に追加。`BlendMode` は既存（**`MaterialTypes.h:198`**、`RenderTypes.h` ではない点に注意）を再利用。**`UVRect` は現在のコードに存在しない**（`struct UVRect` は 0 ヒット）ため、`RenderTypes.h` に最小定義を新設する: `struct UVRect { float U0, V0, U1, V1; static constexpr UVRect FullTexture() { return {0,0,1,1}; } }`（正規化 [0,1] テクスチャ座標）。
...
**GPU instance data の扱い（レビュー major 反映・F3 で確定）:** `BoardProxy` は **host-side proxy**（GPU へ送らない、`MeshProxy`/`LightProxy` と同じホスト proxy パターン）。GPU へ渡る per-instance データは `FramePacket.InstanceData`（既存 flat array）へ append する。F3 は **既存 `GPUSceneInstanceData` レイアウトを流用**するか、専用 `Board2DInstanceData{ float WorldTransform[16]; float Tint[4]; float UVRect[4]; }` を新設するかを F0 で既存 `InstanceData` レイアウトを実コード確認のうえ F3 で確定する（GLSL std140/std430 と C++ `alignas(16)` を一致させレイアウトテストで固定）。`SortKey`/`LayerPriority` は **ホスト側のソート専用**で GPU には送らない（append 順＝描画順がそのまま painter 順）。

### 3.4 共有カメラ（CameraId 参照・スナップショット値コピー維持）

`CameraProxy::CameraId`（`SceneProxy.h:259`、現状未使用）を有効化し、`RenderingCoordinator` に小さなカメラ表を持たせる。`Viewport` は `CameraProxy` を値所有し続ける**が**、authoring の正準は CameraId 経由の lookup とする（複数 Viewport が同一カメラを共有可能にする最小機構）。
...
- **`Viewport` の CameraId 連携（値所有は不変）:** `Viewport::m_Camera`（`Viewport.h:197`）の**型・値所有・コピー操作は一切変えない**（`SetCamera(const CameraProxy&)` `Viewport.h:82`・`GetCamera()` `:87` の値契約を維持）。新規に **`Viewport::SetCameraId(uint64_t)`/`GetCameraId()`**（`m_CameraId` フィールド追加）を**併設**する。`m_Camera`（値）が **primary**、CameraId 参照は **auxiliary**（`BuildViewportRenderPlan` の lookup 時のみ使用）。`SetCamera(proxy)` は後方互換のため、内部で `RegisterCamera(proxy)` して `SetCameraId` も更新する実装としてよい（lookup 経路と値経路を一致させる）。これにより既存 API 契約と新カメラ表共有が矛盾しない。
- **CanvasView 生成と RegisterCamera の呼び出し主体（F1/F2）:** `RenderingCoordinator::CreateCanvasView(uint32_t renderWidth, uint32_t renderHeight, RenderLayer cullingMask)` が CanvasView を生成し、その中で `RegisterCamera({Orthographic, cullingMask=RenderLayer::UI, OrthoWidth=renderWidth, OrthoHeight=renderHeight, Near=0, Far=1})` を呼んで返り CameraId を保持、Viewport を 1 つ構築して `Viewport::SetCameraId(cameraId)` を呼ぶ。`renderWidth/renderHeight` は `RenderingCoordinator` が既知（`m_RenderWidth/m_RenderHeight`）。リサイズ時は `UpdateCamera` で OrthoWidth/Height を追従させる。`IsCanvasViewEnabled()`/`GetCanvasView()` も併設し、ルーティング Step1 で CanvasView 登録有無を確認できるようにする。生成は **pre-frame 専用**で、GameThread 上の coordinator / RenderWorld 初期化後・最初の `BeginFrame` 前に限る。`m_bFrameSubmissionStarted` と、利用可能なら FramePacket state（Writing/Queued/Reading 等でないこと）で enforce し、フレーム提出開始後の追加登録は assert/false で拒否する。
...
`SyncEntityRecursive`（`World.cpp:921-1062`）の `CastTo<BoardComponent>` 分岐で `BuildBoardProxy` を呼び `BoardProxy` を生成し、**`Space` だけ**で proxy ストアへ振り分ける（カメラ・マスクは見ない）:
- `Space == ScreenSpace` → **CanvasView の `m_BoardProxies`** へ `UpdateBoardProxy`（ObjectId/ComponentId キー）。
- `Space == WorldSpace`（F9） → **SceneView の `m_BoardProxies`** へ（`MeshProxy` 経路とは別ストア）。
- live-set 登録（`liveBoardObjectIds`/`liveBoardComponentIds`）。
- **CanvasView 未登録時のフォールバック:** ScreenSpace Board は **stale 回収から漏らさず live-set には載せるが、CanvasView ストアが無いので proxy を捨てる（silent drop、assert しない）**。F1 のフォールバックで present は不変。

**`SyncEntityRecursive` が CanvasView ストアへ到達する手段（依存方向の裁定）:** World は `RenderingCoordinator` を import しない。代わりに `m_SceneView` と同様、**World が所有/参照する「ルーティング先テーブル」を `SyncToSceneView` の呼び出し側（`RenderWorld`/`RenderingCoordinator`）から渡す**。具体的には `SyncToSceneView` に「ScreenSpace Board の publish 先となる CanvasView の `m_BoardProxies` への登録インターフェース（軽量ハンドル or コールバック）」を引数追加する（World→Rendering の依存を増やさず、呼び出し側が結線）。F3 では「主 SceneView + 1 CanvasView」の 2 固定なので、`RenderingCoordinator` が `World::SyncToSceneView(materials, canvasBoardSink)` の形で CanvasView ストアを注入する。実装は F3 計画レビューで「コールバック注入 vs CanvasView ポインタ受け渡し」のいずれかに確定する（どちらも World が Rendering の具象 View 型へ依存しない形を選ぶ）。

#### Step2: マスク交差による可視判定（`RenderingCoordinator::GenerateDrawCommands`、GameThread）

`GenerateDrawCommands`（`:804` View ループ）の **CanvasView 分岐**（`:818` の SceneView cast に `else if (DynamicPointerCast<CanvasView>)` を並置）で、当該 Viewport の `Camera.CullingMask`（`SceneProxy.h:280`）と各 `BoardProxy.LayerMask` の `HasFlag` 交差（`RenderTypes.h:214`）を取り、**交差した Board のみ**を可視集合として `PrepareBoardDrawCommands` に渡す。交差しない Board はストアに残るが当該 viewport では描画されない（live-set からは外さない）。**ここで初めてカメラを読む**（camera を所有する `RenderingCoordinator` のスコープ内）。

#### 二重描画防止（調査の CRITICAL）

SceneView は従来どおり `MeshProxy` を無条件収集する。`BoardProxy` は **SceneView の `MeshProxy` 経路には絶対に載せない**（ScreenSpace は CanvasView の `m_BoardProxies`、WorldSpace は SceneView の**別ストア** `m_BoardProxies`）。同一 ObjectId の Board が ScreenSpace/WorldSpace 両方へ入ることは無い（Space は単一属性）。`RemoveStaleBoardProxies` は ScreenSpace=CanvasView 側・WorldSpace=SceneView 側それぞれが ObjectId/ComponentId キーで独立に回収する。
...
- **採用 (a)：完全オーバーライド。** `CanvasView::Render(ViewRenderContext&)` は `View::Render`（`View.cpp:173`）を**オーバーライドし基底を一切呼ばない**。内部で `context.Graph->Reset()`（`View.cpp:195` 相当を自前で実行）→ 2D Board 描画パス（深度なし quad painter、`BlendMode` ごとに blend state、`PolygonMode::Fill`）を `AddPass` → `Compile`/`ExecuteWithResult` し、出力を `CanvasView::m_OutputTexture`（物理 RT）へ store し **`Canvas.Color`** 名でも publish する。Board 0 個でも RenderGraph パスを実行し、透明 clear/store 済みの物理 `Canvas.Color` を確定する（宣言だけでは不可。null-pipeline の fullscreen/render-pass command などで `BeginRenderPass`/`EndRenderPass` の clear/store を実際に発行）。`context.PresentationGraphPass` は**末尾に追加しない**（= 2D View は直接 present しない。段2 が present を一括で行う）。
...
これを `bUseCompositePass` で分岐する 2 段へ**再構成**する（既存 helper `ApplyViewportRenderPlan`/`ConfigurePresentationGraphPass`/`RenderViewForCurrentViewport`/`FlushPendingFrameCommands` を再利用。ただし zero-viewport CanvasView は direct render する）:
...
Execute(request):
  ...
  for viewPlan in sortedViews:
    if view is enabled CanvasView && viewPlan.Viewports.empty():
      ConfigurePresentationGraphPass(request, /*present=*/nullptr)
      CanvasView::Render(*context)                         // RenderViewForCurrentViewport をバイパス
...
正準ケース:
  ...
  enabled CanvasView（viewport 0 個） → true
```
...
- **レイヤー内:** `OrderInLayer`（append 順）で安定。
...
- **`OrderInLayer` 採番の所有者と寿命（確定）:** prefab レベルでは `BoardComponent.OrderInLayer` を `PROPERTY` で指定（既定 0）。シーン実行中に動的追加する場合は **CanvasView 単位の追加カウンタ**（CanvasView が `m_NextOrderInLayer` を持ち、`AddBoard` 毎に `SetOrderInLayer(m_NextOrderInLayer++)`）で auto-increment。**Remove で詰めない＝欠番許容（sparse）**。フレーム間安定性は「append 順＝採番順」で定義される。グローバルでなく CanvasView 単位なので複数 CanvasView でも衝突しない。
- **SortKey 衝突と stable_sort 規約（確定）:** 同 `LayerPriority` かつ 同 `OrderInLayer` のときのみ SortKey が一致する（auto-increment で実用上発生しない）。`std::stable_sort` は比較が等価（同 SortKey）の要素の入力順を保存する。万一衝突しても **append index（可視集合への投入順）でタイブレーク**して決定的にする（比較子で SortKey 一致時 index 比較）。`Container::VariableArray` のランダムアクセス/イテレータ互換は F0 で確認し、`std::stable_sort` 非対応なら `std::vector` 中継 or index タイブレークの自前安定ソートで代替（F4 で実装）。
...
CanvasView の各 Viewport は `CameraProxy{ Projection = Orthographic, CullingMask = RenderLayer::UI }` を CameraId 共有で持つ。`Viewport::UpdateMatrices`（`Viewport.cpp` の Orthographic パス）が `OrthoWidth/OrthoHeight` から正射影行列を生成する。
...
- **(b) 原点と y 軸（Vulkan 規約を正準候補・F0 で確定）:** 正射影行列 `CreateOrthographic(left=0, right=renderWidth, top=renderHeight, bottom=0, near=0, far=1)`（左上原点・y 下＝Vulkan 既定）。Board `(0,0)` は画面左上、`(renderWidth, renderHeight)` は右下。**F0 で `MatrixUtils::CreateOrthographic` の実装（引数順・Row/Col major・利き手）と既存 `Viewport::UpdateMatrices` の Orthographic パスを実測**し、左上原点・y 下が成り立つ引数を確定して `2d-rendering-baseline.md` に記録、`board2d.vert` の MVP 計算に反映する。
...
- **(d) F3-F8 はスクリーン全面 Canvas（Viewport rect=(0,0,1,1)）を前提。** 全面では `OrthoWidth = renderWidth`、`OrthoHeight = renderHeight` でスクリーン px 座標と一致。リサイズ時は `UpdateCamera` で OrthoWidth/Height を追従（§3.4.1）。
...
- **検証:** Board を `(0,0)` に置き全面を埋める／`(renderWidth/2, renderHeight/2)` に置き中央配置されることを `OrthographicViewportMatrixTest` で固定（F0 で行列正準を記録 → F3 で配置を確認）。
...
- **live-set キーイング（確定）:** `BoardProxy` は `ObjectId`（所属 Entity）+ `ComponentId`（BoardComponent）の両方を持つ。ストア index は `MeshProxy` 同様 ObjectId/ComponentId キーの `UnorderedMap`。`RemoveStaleBoardProxies` は live-set（`liveBoardObjectIds`/`liveBoardComponentIds`）に無い proxy を回収。1 Entity に複数 BoardComponent が付きうるため、回収判定は **ComponentId キー**を主に使う（`MeshProxy` の ObjectId 主キーと使い分け：Mesh は 1 Entity 1 Mesh 前提、Board は複数可のため Component 粒度）。
- **スレッド・FramePacket 不変性（レビュー major 反映）:** ルーティング・ソート・カメラ表 lookup は GameThread。RenderThread は FramePacket スナップショット（共有 flat array + `ViewportRenderPlan`）のみ。
  - **値のみ・ポインタ禁止:** `BoardProxy` および FramePacket へ載る全 Board データは **値（transform 行列・`TextureHandle`・tint・UV）のみ**。`BoardComponent`/`Entity`/`BoardProxy` の**ポインタを snapshot に載せない**（既存 `MeshProxy`/`LightProxy` パターン踏襲）。よって in-flight 中に Board entity が delete されても RenderThread は stale ポインタを踏まない。
...
推奨フェーズ順序の根拠: まず合成段の骨格（F1）を 2D 空で通して既存挙動不変を担保 → カメラ共有 + 正射影 Viewport（F2）→ BoardComponent/BoardProxy/ルーティング/最小描画（F3）→ 順序担保（F4）→ 見た目機能（F5-F7）→ 性能（F8）→ world-space Billboard（F9）→ 方式 B（F10）→ Impostor（F11）と、各段が独立してビルド・コミットできる順に積む。
...
- **World→Proxy 同期の BoardProxy 追加点**を確認: `SyncEntityRecursive`（`World.cpp:921-1062`）の Component 振り分け（`:943-1026`）と live-set/RemoveStale（`:870-872`）、`BuildMeshProxy`（`MeshComponent.cpp:186-247`）のテンプレート。BoardComponent 分岐とルーティングを足す正確な行を特定する。
- **RenderLayer-CullingMask の現状使用と交差ルーティングの配線規模**を確認: `CameraProxy.CullingMask`（`SceneProxy.h:280`）がカリングに未使用であること、`HasFlag`（`RenderTypes.h:214`）が使えること、`MeshProxy.LayerMask`（`:87`）の設定経路（`MeshComponent.cpp:238`）。
- **カメラ共有化の影響範囲**を確認: `CameraProxy.CameraId`（`SceneProxy.h:259`）未使用、`Viewport` の値所有（`Viewport.h:87`）、`SetMainCamera`/`m_MainCamera`（`RenderingCoordinator.h:219,354`）、`BuildViewportRenderPlan` のフォールバック（`RenderingCoordinator.cpp:863-864`）。値コピー契約（`ViewRenderPlan.h:32`）を壊さない範囲を確定。
...
- **合成側アンカー**を確認: `View::Render` の `Reset`（`View.cpp:195`）・`PresentationGraphPass` 条件（`:281-284`）、`PresentationPass::Declare` 優先順（`PresentationPass.cpp:27-40`）、`RenderFrameExecutor::Execute` 二重ループ（`RenderFrameExecutor.cpp:29-75`）、`RenderGraphResourceNames.h`（`Composite.Color`/`Canvas.Color` が無いこと）。
...
- `ProceduralMeshGenerator`（quad 生成が無いこと `ProceduralMeshGenerator.h`、`GenerateUVSphere:34`/`GeneratePlane:158`、winding 実測）、`VertexLayout` の 2D プリセット有無、`TextureHandle`/`BlendMode`（`MaterialTypes.h:198`）/`UVRect`（**未定義の確認**）の既存定義有無を確認。
...
- `Docs/Plans/2d-rendering-baseline.md`（新規、プレーン UTF-8）に: ベースライン描画統計・PSO 数・validation、上記確定項目（BoardProxy 追加点 / マスクルーティング配線規模 = 2 ステップ分割の Step1/Step2 配線位置 / カメラ共有影響範囲 / Board 振り分け点（`:818` の else-if 位置）/ 合成側アンカー / SortKey 割付 + 安定ソート可否 / ScreenSpace transform（原点/y 軸/MVP 順）/ 正射影正準（`CreateOrthographic` 引数規約）/ GenerateQuad2D winding / UVRect 未定義 / CanvasView side-effect / triple-buffer 寿命）、`board2d.vert` 用の決定値、Normal 代表スクショ（`Docs/Plans/assets/2d-rendering/`）。
...
- 新規 `Library/Core/Public/Rendering/CanvasView.h` / `Private/Rendering/CanvasView.cpp`: `CanvasView : public View`、`Initialize` で `ViewType::UI`、`Render(ViewRenderContext&)` を**完全オーバーライド**（基底を呼ばず `Reset` → 空（透明クリア）の `m_OutputTexture` を確定し `Canvas.Color` publish → `PresentationGraphPass` 追加しない、§3.6.1）。透明 Canvas は RenderGraph pass を実行し、null-pipeline fullscreen/render-pass command 等で `BeginRenderPass`/`EndRenderPass` の clear/store を物理発行する（宣言のみ不可）。**F1 の CanvasView は正射影 Viewport を持たない骨格**（カメラ表が無いため）。Board 描画も無く、zero-viewport でも `CanvasView::Render` を直接呼び、`RenderViewForCurrentViewport` をバイパスして透明な `m_OutputTexture` を出す。正射影 Viewport の追加は **F2**（カメラ表導入後）。これにより F1→F2 の依存が一方向に保たれる（F1 は Viewport/カメラに依存しない空 Canvas、F2 でカメラ表 + 正射影 Viewport を足す）。
- 新規 `CompositePass.h`/`CompositePass.cpp` + `composite2d.vert`/`composite2d.frag`（BOM なし）: `ImportTexture(SceneView::m_OutputTexture)`（物理 RT、named resource ではない）+ 任意 `ImportTexture(CanvasView::m_OutputTexture)` → alpha-over（または canvas 無し時 scene 素通し）→ `Composite.Color` を**常に publish**（§3.6.2/§3.6.3）。
...
- 新規テスト: `RenderGraphResourceNamesTest`（Canvas/Composite 追加）、`CompositePassPassthroughTest`（Canvas 無し時 scene 素通し）、`CompositePassInputSourceTest`（段2 が persistent RT を import・named resource を段1 と共有しない、§3.6.2）、`RenderFrameExecutorTwoStageTest`（段1/段2 シーケンス、zero-viewport CanvasView direct render）、`RenderFrameExecutorFallbackTest`（`ShouldCompose` false → 従来 `:29-75` 経路でビット不変、§3.6.5）、`ShouldComposePredicateTest`（1 Scene=false、2 SceneViews=false、disabled Canvas=false、enabled zero-viewport Canvas=true）、`CreateCanvasViewLifecycleTest`（pre-frame のみ許可）、`FrameOutputLifetimeTest`（render start/failure reset・段2 request/result clear）。

### ゲート

- focused build（Game）、CTest 全通過（新規含む）、**(1) CanvasView 未登録/disabled 時は描画ビット完全不変**（従来経路、MT/ST スクショ）、**(2) enabled CanvasView 登録・Board 0・zero-viewport 時は視覚不変**（段2 経路、透明 Canvas 合成、MT/ST スクショ pixel 一致）、ShouldCompose 4 ケース通過、Canvas clear/store が物理実行されること、Vulkan validation なし、`git diff` 行末破壊なし、エンジンクリティカル実装レビュー完了。
...
`CameraProxy.CameraId`（`SceneProxy.h:259`）を有効化し、`RenderingCoordinator` にカメラ表（`RegisterCamera`/`UpdateCamera`/`FindCamera`）を導入する。Viewport は CameraId 参照で共有カメラを引く（値所有は維持、スナップショット値コピーも維持）。CanvasView に正射影カメラ Viewport（`CullingMask=RenderLayer::UI`、px 空間）を持たせる。
...
- `RenderingCoordinator` にカメラ表メンバ（`UnorderedMap<uint64_t, CameraProxy> m_Cameras`、`m_NextCameraId`、`m_MainCameraId`）と `RegisterCamera`/`UpdateCamera`/`FindCamera` を追加（§3.4、新 Manager を作らない）。`SetMainCamera`（`RenderingCoordinator.cpp` の実装）は `m_MainCameraId` を再利用して overwrite（Game 側は ID を意識しない後方互換）。
- `Viewport` に CameraId 参照を追加（`SetCameraId`/`GetCameraId`）。`SetCamera`（`Viewport.h:82`）API は維持。`BuildViewportRenderPlan`（`RenderingCoordinator.cpp:40-115`）で CameraId からカメラ表 lookup → 解決した `CameraProxy` を `ViewportRenderPlan.Camera`（`ViewRenderPlan.h:32`）へ**値コピー**（フォールバックは従来の `m_MainCamera` `:863-864` を維持）。
- `CanvasView` に正射影カメラ Viewport を 1 つ作る: `RegisterCamera({Projection=Orthographic, CullingMask=RenderLayer::UI, OrthoWidth=renderWidth, OrthoHeight=renderHeight, NearPlane=0, FarPlane=1})` の CameraId を Viewport へ設定。`Viewport::UpdateMatrices` の Orthographic パスで正射影行列生成（§3.8）。

### 成果物

- カメラ表 + CameraId 参照、`Viewport` CameraId 連携、`BuildViewportRenderPlan` の lookup、CanvasView 正射影 Viewport。
- 新規テスト: `CameraTableShareTest`（複数 Viewport が同一 CameraId を共有し同一スナップショットを得る）、`CameraTableLifetimeTest`（採番 1 始まり・0=invalid・lookup 失敗時 `m_MainCamera` フォールバック・`SetMainCamera` の `m_MainCameraId` 再利用、§3.4.1）、`OrthographicViewportMatrixTest`（px 空間正射影行列の正準固定・`(0,0)` 全面/`(w/2,h/2)` 中央、§3.8）。

### ゲート

- focused build（Game）、CTest 全通過、**3D 描画結果が不変**（主 SceneView は従来カメラ経路、MT/ST スクショ比較）、エンジンクリティカル実装レビュー完了。
...
`BoardComponent`（Entity Inner、`REFLECTION_CLASS`/`PROPERTY`）、`BoardProxy`、`SyncEntityRecursive` の BoardComponent 分岐、`RenderLayer マスク × CullingMask` ルーティングで CanvasView へ振り分け、単一 ScreenSpace Board の最小描画（正射影・方式 A）を実装する。
...
- `RenderTypes.h` に `enum class BoardSpace`、**`UVRect` の新定義**（未定義のため、§3.3）、`BlendMode` は既存（`MaterialTypes.h:198`）を include して再利用。`SceneProxy.h` に `BoardProxy` 追加（§3.3）。
...
- **CanvasView に `BoardProxy` ストア** + 登録 API（`AddBoardProxy`/`UpdateBoardProxy`/`RemoveStaleBoardProxies`、ObjectId/ComponentId キー、`SceneView.h:87-160` の MeshProxy パターン踏襲）。BoardProxy は **CanvasView 側にのみ保持**（SceneView の MeshProxy 経路に載せない、§3.5 二重描画防止）。F3 は ScreenSpace のみなので SceneView 側 Board ストアは F9 まで作らない。
- **Step1（`World::SyncEntityRecursive`、`World.cpp:921-1062`、カメラ非依存）:** `CastTo<BoardComponent>` 分岐を追加: dirty 判定（`IsRenderStateDirty()`/transform version、`:948-949`）→ `RefreshRenderTransformCache` → `BuildBoardProxy` → ObjectId/ComponentId 設定 → **Space だけで振り分け**（`Space==ScreenSpace` → 注入された CanvasView Board sink へ `UpdateBoardProxy`、§3.5 Step1。**ここではマスク交差を見ない**）→ live セット登録。`SyncToSceneView`（`:834`、RemoveStale は `:870-872`）に `RemoveStaleBoardProxies(liveBoardComponentIds)` を追加し、CanvasView Board sink を引数注入（World は Rendering 具象型に依存しない sink/コールバック、§3.5）。
- **Step2（`RenderingCoordinator::GenerateDrawCommands`、`:804` View ループ、カメラを読む）:** `:818` の `DynamicPointerCast<SceneView>` に `else if (DynamicPointerCast<CanvasView>)` 分岐を並置し、`CanvasView::PrepareBoardDrawCommands(viewportPlan)` を呼ぶ。その中で `HasFlag(viewportPlan.Camera.CullingMask, board.LayerMask)` 交差で可視集合を作り（§3.5 Step2）、`ComputeSortKey` で安定ソート（F4 で完全化、F3 は単一 Board なので順不問）→ 共有 quad mesh（`ProceduralMeshGenerator::GenerateQuad2D`、**CCW winding** §3.3）で quad draw/instance を `FramePacket.DrawCommands`/`InstanceData` へ **3D の後に** append → `viewportPlan.DrawCommandRange` を設定。
- `ProceduralMeshGenerator::GenerateQuad2D`（2 三角・UV 付き・CCW、§3.3）、`VertexLayout` の 2D プリセット（Position2D+TexCoord+Color）、`board2d.vert`/`board2d.frag`（BOM なし、深度なし PSO、§3.8 の px 空間・原点/y 軸/MVP 順は baseline.md の確定値に従う）。CanvasView の Board 描画パスで `DepthStencilState{depthTest=false, depthWrite=false}`。

### 成果物

- `BoardComponent`/`BoardProxy`/`BoardSpace`、SyncEntityRecursive 分岐 + マスクルーティング、CanvasView BoardProxy ストア + `PrepareBoardDrawCommands`、quad mesh/2D VertexLayout/board2d シェーダー、単一 ScreenSpace Board の描画。
- 新規テスト: `BoardComponentReflectionTest`（PROPERTY round-trip = prefab シリアライズ確認）、`BoardProxyRoutingTest`（Step1 Space 振り分け + Step2 マスク交差で CanvasView 可視・SceneView の MeshProxy へ漏れない、§3.5）、`BoardProxyKeyingTest`（ObjectId/ComponentId キー・stale 回収）、`BoardComponentTransformTest`（`RefreshRenderTransformCache` が Entity world transform を px としてキャッシュ、§3.2/§3.8）。Game テストシーンに 1 枚の ScreenSpace Board を表示し `F3_expected.png` で目視/pixel 比較。

### ゲート

- focused build（Game）、CTest 全通過、**3D 描画不変 + 画面に 1 枚の Board が正しい px 位置/サイズで表示**（MT/ST スクショ比較）、二重描画なし、エンジンクリティカル実装レビュー完了。

### コミット例

```text
BoardComponentとBoardProxyを追加しRenderLayerマスク交差でCanvasViewへ振り分けて単一Boardを描画する
```
...
複数 Board の painter 順描画を、(a) レイヤー間 = `LayerPriority` 昇順、(b) レイヤー内 = `OrderInLayer` 安定の 2 階層で担保する。`CanvasView::PrepareBoardDrawCommands` で明示安定ソートする（§3.7）。
...
- `CanvasView::PrepareBoardDrawCommands` で BoardProxy を `ComputeSortKey`（`(LayerPriority<<32)|OrderInLayer`）で `std::stable_sort`（F0 で `VariableArray` イテレータ互換確認済み）。非対応なら同 SortKey を append index でタイブレークして安定化。`uint32` 上限 `assert`。
- `BoardComponent` の `LayerPriority`/`OrderInLayer` PROPERTY を有効化（採番方針: `OrderInLayer` は追加順 auto-increment、Remove で詰めない=フレーム間安定）。
...
F0(read-only)
 └─ F1(合成骨格・2D空) ── F2(カメラ共有+正射影Viewport)
                              └─ F3(BoardComponent/Proxy/ルーティング/最小描画)
                                   └─ F4(複数Board・2階層安定ソート)
...
順序根拠: 合成段の骨格（F1）を 2D 空で通して既存挙動不変を最初に固める → カメラ共有（F2）で CanvasView の正射影 Viewport の基盤を用意 → BoardComponent/Proxy/ルーティング（F3）でオーサリング経路を確立 → 順序担保（F4）→ 見た目機能（F5-F7）→ 性能（F8）→ world-space（F9）→ 方式 B（F10）→ Impostor（F11）。各段が独立してビルド・レビュー・コミットできる最小単位。
...
| RenderLayer マスク × CullingMask ルーティング | 採用 | F3 | `CameraProxy.CullingMask`(`SceneProxy.h:280`) × `BoardProxy.LayerMask`、`HasFlag`(`RenderTypes.h:214`)、`SyncEntityRecursive`(`World.cpp:921`) |
...
| 1 段合成（2D over 3D） | 採用 | F1 | `CompositePass`(新規)、`PresentationPass`(`PresentationPass.cpp:22`)、`RenderFrameExecutor`(`RenderFrameExecutor.cpp:11`) |
...
- **共有ファイルは単一オーナーまたはオーケストレーターが順次編集**する。共有ファイル: `SceneProxy.h`（BoardProxy）/ `World.h`/`World.cpp`（SyncEntityRecursive・ルーティング）/ `RenderingCoordinator.h`/`RenderingCoordinator.cpp`（カメラ表・GenerateDrawCommands）/ `ViewRenderPlan.h` / `FramePacket.h` / `RenderGraphResourceNames.h` / `PresentationPass.cpp` / `RenderFrameExecutor.cpp` / `Viewport.h`/`Viewport.cpp` / `RenderTypes.h` / `Library/Core/CMakeLists.txt`。サブエージェント間で書き込みスコープを重複させない。
