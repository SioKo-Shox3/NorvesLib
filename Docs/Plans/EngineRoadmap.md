...
作成日: 2026-07-02 ／ 最終更新: 2026-07-18 ／ ステータス: 進行中（M1・M3・M4完了、次はM2）
...
- **M2「viewport.getThumbnail」は main 未統合**。`feature/viewport-thumbnail` に6コミット・40ファイルの部分実装があるが、main より45コミット遅れており、そのまま merge してはならない。現 main へ移植し、readback から E2E まで再検証する必要がある（E2E は未確認）。
- **実行順と依存関係（再確認）**: 当初順は M2 → M3 → M4 → M5 だったが、M4 を先行消化した。M3/M4 は完了済みであり、現在の次工程は **M2**、M5 は M2 後とする。
  | 順 | M | 依存 | 独立性 | 備考 |
  |---|---|---|---|---|
  | 次 | M2 ビューポート | なし（RHI/Rendering 単独） | 高 | 旧 branch の部分実装を現 main へ移植・再検証し、E2E を完了する |
  ...
  | M2後 | M5 テキスト/パーティクル/デバッグUI | M3 の絵作り土台があると映える | 中 | 各項独立フェーズ |
...
| ビューポート連携 | `feature/viewport-thumbnail` に readback・キャプチャ・PNG・Bridge の部分実装があるが、main 未統合。エディタ側仕様は確定済み（base64 PNG 640x360 ≤1fps pull） | 現 main への移植と再検証、特に E2E 未確認の完了 |
...
| M2 | エンジンが見えるエディタ — viewport.getThumbnail | Editor連携 | ▶ 次（main未統合。旧branchに部分実装、現mainへ移植・再検証が必要） |
| M3 | 動くライトと正しい影 — シャドウ結線・ライト拡張 | レンダリング | ✅ 完了（main統合済み） |
| M4 | アセットが流れるエンジン — メッシュクック＋リロード最小 | アセット | ✅ 完了（`fddbba0e`で閉鎖、受容済みnon-blockingリスクあり） |
| M5 | デモが映える画面 — テキスト描画・パーティクル・デバッグUI | レンダリング/2D | 未着手 |
...
依存関係: M1→M6（ScriptComponent のプロパティ保存）、M7→M8（物理は固定ステップ前提）以外は疎。M1〜M4 はオーケストレーション運用下で一部並行可能。ただし共有ファイル（公開ヘッダ・中央CMake）の編集は単一オーナー原則を守ること。
