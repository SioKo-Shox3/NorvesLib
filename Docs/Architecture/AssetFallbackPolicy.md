...
Phase 16A adds prepared cooked texture loading through `TextureResources`, reached from `GEngine->GetRenderResources().Textures()`. The prepared API is cooked-only: it resolves manifest entries, reads the package, parses `nvtex`, and returns a prepared payload for later render-side finalization. It does not loose-read or decode an image file.
