...
- `ObjectHandle` は Object lifetime と generation を検証するための長期参照であり、raw pointer の保存代替にする。
- `ObjectRef<T>` は GC の strong reference、`WeakObjectRef<T>` は mark しない weak reference として扱う。
- `Outer` / `Inner` は gameplay hierarchy と名前空間を表す。所有権の実体は `ObjectHeap`、到達可能性は GC traversal が担当する。
...
`PropertyBag` は値の byte 列だけではなく、型ごとの操作を持ちます。
...
このため `PropertyBag` に入れられる型は、少なくとも copy / destroy / serialize policy を登録できる型に限定します。Object 参照、Resource 参照、配列、文字列、ユーザー定義 struct はこの `TypeOps` 経由で扱います。
...
- `TypeId`、`PropertyValue`、`TypeOps` を導入する。
...
- `Clone()` を基底 API から外す。
- CDO 的 default object の必要性を再評価し、Class defaults descriptor へ置き換える。
