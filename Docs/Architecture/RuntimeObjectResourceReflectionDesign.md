...
`StableObjectId` は保存データや外部 protocol 用の識別子です。`ObjectPath` は人間が読める debug / fallback key、`SceneObjectId` は scene/prefab 内の局所 ID として扱います。
...
- `Outer -> Inner` は既定で strong edge として mark する。
...
`InvokeTargetKind` は `Object`、`Resource`、`Subsystem`、`Global` を持ちます。これにより Object の member command、Resource の reload/reimport、Engine subsystem command、global debug command を同じ invocation path で扱えます。
...
- `StableObjectRef` と `ObjectResolver` を追加する。
...
- `InvokeTarget` を Object / Resource / Subsystem / Global の tagged union として定義する。
...
- `StableObjectId` は project-wide GUID にするか、scene/prefab 局所 ID + path にするか。
