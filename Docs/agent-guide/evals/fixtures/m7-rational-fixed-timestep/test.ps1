param(
    [string]$SourcePath = (Join-Path $PSScriptRoot 'FixedStepScheduler.cpp')
)
...
$temporaryRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("m7-rational-fixed-timestep-" + [System.Guid]::NewGuid().ToString('N'))
try {
    ...
    Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'FixedStepSchedulerTest.cpp') -Destination (Join-Path $temporaryRoot 'FixedStepSchedulerTest.cpp')
    Copy-Item -LiteralPath $resolvedSourcePath -Destination (Join-Path $temporaryRoot 'FixedStepScheduler.cpp')
...
Write-Host 'm7-rational-fixed-timestep passed'
