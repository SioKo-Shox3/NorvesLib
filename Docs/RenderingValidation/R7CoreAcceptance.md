...
| SPP収束・公開Cornell | R7-P4（`2365a2c`・`944452e`）。`PathTracingConvergenceVulkanTest`（`.harness/runs/20260924-r7-p4/ctest-1.txt`）。**収束は合格**: 本番設定（本番BSDF・MIS・画素内一様標本、環境光なし）のCornell boxで、同じ乱数列の入れ子の接頭列の256 spp画像に対するMSEは16 spp `0.00783138`→64 spp `0.00159723`→256 spp `0`と単調に減り、64x64の64区画すべてで単調、区画ごとのMSE(16)/MSE(64)の中央値`4.374`（独立な試料の期待値5、事前固定の範囲4〜6.25）。履歴を捨てて引き直した16 sppはbyte一致。**公開Cornellは未合格**: 純Lambert・4096 sppは影床ROIの相対誤差`0.0025`、区画の中央値`0.0176`、発光面の範囲のずれ1画素で一致するが、赤・緑ROIの相対誤差`0.1047`・`0.1190`（事前固定の上限0.10）と区画の90%点`1.551`（上限0.20）が超過する。超過は赤・緑の壁の区画に限られ、原因はPTの輸送ではなく、R4 fixtureのRGB入力と公開RGBEの色の符号化の違い（下の「公開Cornell参照とR1数値契約」）。 |
