...
| 画像 | 内容 | 平均輝度 |
|---|---|---|
| raster-rtgi | R6 RTGI。履歴の最大age（8 rendered frame）の3倍の24 frame後に取得 | 0.259357（R6受入れの静止段階の記録値0.2594と一致） |
| pt-direct | PT（`--path-tracing-transport=direct`、16384試料以上）。1次命中の発光と直接光だけ | 0.334121 |
| pt-single | PT（`single-diffuse-bounce`）。R6の申告範囲（拡散1バウンス）に合わせた参照 | 0.384239 |
| pt-full | PT（`full`）。多重散乱をすべて追う参照 | 0.437101 |

判定の参照はR6の申告範囲に合わせた`pt-single`とし、閾値は比較の実行前に`6a03e1b`で固定した。全画素のFLIP平均と、8×8区画平均画像のFLIP最大が、参照の間接光成分（`pt-single`−`pt-direct`）を一様に±20%変えた画像と参照との知覚差（小さい方）以下であることを求める。R4 DDGIの受入れ（ROI平均25%）より厳しく、正しい推定の残留雑音は許容する大きさとして選んだ。±40%の変化が閾値の外にあることで物差しの単調性を確かめる。

| 比較 | FLIP平均 | 8×8区画FLIP最大 | 判定 |
|---|---|---|---|
| 閾値（間接光+20%、−20%の小さい方） | 0.0816135 | 0.166251 | — |
| 物差しの単調性（間接光+40% / −40%） | 0.130176 / 0.148028 | 0.260045 / 0.323719 | 閾値の外 |
| R6 RTGI 対 `pt-single` | 0.701434 | 0.965484（区画(28,13)） | **超過** |
| 陽性対照: 155試料の`pt-single` 対 `pt-single` | 0.0266482 | 0.0320118 | 閾値内 |
| 参考: `pt-full` 対 `pt-single`（R6の範囲外の多重散乱） | 0.254082 | 0.570346 | — |
| 参考: R6 RTGI 対 `pt-full` | 0.782309 | 0.968766 | — |

R6の画像の平均輝度はPTの直接光だけの画像より低い（間接光に当たる差が−0.0748）。ラスタのLightingPassは発光三角形を光源として扱わず、天井の面光源の直接光はRTGIのコサイン標本が偶然面光源へ当たったときだけ入る。面光源は小さく非常に明るいため、1試料/frame・8 frameの履歴・3×3 filterでは斑点状の雑音として残り、壁と箱の照明がほぼ欠ける（`.harness/runs/20260924-r6-p5-ref/r6-reference-images.png`）。RTGIの命中点の直接光も点・spot・方向光だけで、面光源に照らされた面からの1バウンスも入らない。これはPT方式との正当な輸送差（多重散乱）ではなく、R6の申告範囲内の欠落である。

Roadmap更新ルール5に従いR6をステータス表で「再オープン」とし、閾値とgoldenは緩めない。修正はR6-P7（RTGIで発光三角形を光源標本する）で行い、同じテストを同じ閾値で再実行する。

| 記録 | 内容 |
|---|---|
| `.harness/runs/20260924-r6-p5-ref/ctest-r6-reference-1.txt` | 取得4回と比較の全出力。EXIT_CODE=8（R6の閾値超過） |
| `.harness/runs/20260924-r6-p5-ref/positive-control-pt-low-spp.txt` | 陽性対照の比較出力 |
| `.harness/runs/20260924-r6-p5-ref/ctest-lighting-transport.txt` | PTの輸送範囲の解析確認（平面1枚で直接光のみ=点光源の解析値、拡散1バウンス−直接光=反射率×環境、誤差5e-6） |
