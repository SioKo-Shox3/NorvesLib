...
namespace
{
    ...
    void TestLensValues()
    {
        ...
        assert(IsNearlyEqual(proxy.Viewport.MinDepth, 0.2f));
        assert(IsNearlyEqual(proxy.Viewport.MaxDepth, 0.8f));
    ...
    // ========================================
    // ケース4a: BuildCameraProxy は IsEnabled/IsActiveCamera で選別しない
    // ========================================
    void TestBuildProxyIgnoresEnabledAndActiveState()
    {
        World world;
        world.Initialize();
        ...
        // 明示的に無効化・非アクティブのままでも BuildCameraProxy は成功し続ける
        // （有効性/アクティブカメラによる取捨選択は呼び出し側の責務であり、
        //  BuildCameraProxy 自体はフィルタしないという契約を回帰検証する）。
        camera->Disable();
        assert(camera->IsEnabled() == false);
        assert(camera->IsActiveCamera() == false);
        ...
        // SetActiveCamera(false) を明示しても同様に成功する
        camera->SetActiveCamera(false);
        CameraProxy proxy2;
        assert(camera->BuildCameraProxy(proxy2));
...
int main()
{
    ...
    TestLensValues();
    TestDefaultValues();
    TestBuildProxyIgnoresEnabledAndActiveState();
