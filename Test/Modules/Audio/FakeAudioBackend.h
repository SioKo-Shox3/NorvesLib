...
namespace NorvesLib::Test::Audio
{
    ...
    public:
        ...
        void EmitBinding(size_t bindingIndex,
                         Modules::Audio::VoiceHandle handle,
                         uint64_t epoch,
                         Modules::Audio::Private::AudioBackendEventKind kind)
        {
            if (EventSink == nullptr || bindingIndex >= Mailboxes.size())
            {
                return;
            }
            FakeAudioMailboxBinding& binding = Mailboxes[bindingIndex];
            if (binding.Mailbox != nullptr)
            {
                EventSink->EnqueueBackendEvent(*binding.Mailbox, handle, epoch, kind);
            }
        }
