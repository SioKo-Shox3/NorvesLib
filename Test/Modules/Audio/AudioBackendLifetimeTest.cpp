...
namespace
{
    ...
    bool TestGenerationExhaustionRetiresVoiceSlot()
    {
        auto backend = MakeUnique<FakeAudioBackend>();
        FakeAudioBackend* fake = backend.get();
        auto module = Private::CreateAudioServiceForBackend(std::move(backend));
        ...
        VoiceHandle initialHandle;
        assert(module->CreateVoice(MakeClip(), initialHandle) == AudioResult::Success);
        assert(initialHandle.Generation == 1);

        VoiceHandle maximumGenerationHandle = initialHandle;
        assert(Private::AudioServiceTestAccess::PrepareVoiceGenerationWrap(
            *module,
            maximumGenerationHandle));
        assert(maximumGenerationHandle.Generation == UINT32_MAX);
        assert(module->StartVoice(maximumGenerationHandle) == AudioResult::Success);
        assert(module->StopVoice(maximumGenerationHandle) == AudioResult::Success);
        fake->EmitBinding(
            0,
            maximumGenerationHandle,
            fake->ShutdownEpoch,
            Private::AudioBackendEventKind::Drained);
        module->Tick();
        assert(module->DestroyVoice(maximumGenerationHandle) == AudioResult::Success);

        VoiceHandle replacementHandle;
        assert(module->CreateVoice(MakeClip(), replacementHandle) == AudioResult::Success);
        if (replacementHandle.Index == initialHandle.Index)
        {
            std::cerr << "exhausted generation slot was reused\n";
            module->Shutdown();
            return false;
        }
        AudioVoiceState state = AudioVoiceState::Destroyed;
        assert(module->GetVoiceState(initialHandle, state) == AudioResult::StaleHandle);
        assert(module->GetVoiceState(maximumGenerationHandle, state) == AudioResult::StaleHandle);
        const uint64_t staleBefore = module->GetDiagnostics().StaleEventCount;
        fake->EmitBinding(
            0,
            initialHandle,
            fake->ShutdownEpoch,
        ...
        fake->EmitBinding(
            ...
            Private::AudioBackendEventKind::Drained);
        module->Tick();
        assert(module->GetDiagnostics().StaleEventCount == staleBefore + 2);
        assert(module->Shutdown() == AudioResult::Success);
        return true;
    }

    bool TestMailboxAllocationIsBoundedByVoiceSlots()
    {
        auto backend = MakeUnique<FakeAudioBackend>();
        FakeAudioBackend* fake = backend.get();
        auto module = Private::CreateAudioServiceForBackend(std::move(backend));
        ...
        constexpr size_t ReuseCount = 64;
        for (size_t index = 0; index < ReuseCount; ++index)
        {
            VoiceHandle handle;
            assert(module->CreateVoice(MakeClip(), handle) == AudioResult::Success);
            assert(module->StartVoice(handle) == AudioResult::Success);
        ...
        const size_t slotCount = Private::AudioServiceTestAccess::GetVoiceSlotCount(*module);
        assert(slotCount == 1);
        const size_t mailboxAllocationCount =
            Private::AudioServiceTestAccess::GetMailboxAllocationCount(*module);
        if (mailboxAllocationCount != slotCount)
        {
            std::cerr << "mailbox allocations=" << mailboxAllocationCount
                      << " voice slots=" << slotCount << "\n";
            module->Shutdown();
            return false;
        }
        assert(module->Shutdown() == AudioResult::Success);
    ...
    bool TestQuarantinedVoiceIgnoresTerminalCallbacksAndCountsDestroyOutcome()
    {
        auto backend = MakeUnique<FakeAudioBackend>();
        FakeAudioBackend* fake = backend.get();
        ...
        constexpr Private::AudioBackendEventKind TerminalKinds[] = {
            Private::AudioBackendEventKind::VoiceError,
            Private::AudioBackendEventKind::Drained,
        };
        for (Private::AudioBackendEventKind kind : TerminalKinds)
        {
            VoiceHandle handle;
            assert(module->CreateVoice(MakeClip(), handle) == AudioResult::Success);
            assert(module->StartVoice(handle) == AudioResult::Success);
            ...
            const uint64_t staleBefore = module->GetDiagnostics().StaleEventCount;
            fake->Emit(handle, fake->ShutdownEpoch, kind);
            module->Tick();
            assert(module->GetVoiceState(handle, state) == AudioResult::Success);
            if (state != AudioVoiceState::Quarantined ||
                module->GetDiagnostics().StaleEventCount != staleBefore + 1)
            {
                std::cerr << "quarantined terminal callback changed state or was not counted stale\n";
                module->Shutdown();
                return false;
            }
        }
        ...
        if (module->GetDiagnostics().QuarantinedVoiceCount != 2)
        {
            std::cerr << "failed DestroyVoice cleared quarantine diagnostics\n";
            return false;
        }

        auto recoveryBackend = MakeUnique<FakeAudioBackend>();
        FakeAudioBackend* recoveryFake = recoveryBackend.get();
        recoveryFake->bFailFlush = true;
        auto recoveryModule = Private::CreateAudioServiceForBackend(std::move(recoveryBackend));
        assert(recoveryModule->Initialize() == AudioResult::Success);
        VoiceHandle recoveryHandle;
        assert(recoveryModule->CreateVoice(MakeClip(), recoveryHandle) == AudioResult::Success);
        assert(recoveryModule->StartVoice(recoveryHandle) == AudioResult::Success);
        assert(recoveryModule->StopVoice(recoveryHandle) == AudioResult::BackendFailure);
        assert(recoveryModule->GetDiagnostics().QuarantinedVoiceCount == 1);
        assert(recoveryModule->Shutdown() == AudioResult::Success);
        assert(recoveryModule->GetDiagnostics().QuarantinedVoiceCount == 0);
        return true;
    }
...
int main()
{
    ...
    if (!TestGenerationExhaustionRetiresVoiceSlot())
    {
        return 1;
    }
    if (!TestMailboxAllocationIsBoundedByVoiceSlots())
    {
        return 1;
    }
    if (!TestQuarantinedVoiceIgnoresTerminalCallbacksAndCountsDestroyOutcome())
    {
        return 1;
    }
